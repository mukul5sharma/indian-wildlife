...
<jsp:useBean id="create" class="beans.CreateTopicBean" scope="request" />
<jsp:useBean id="chooseforum" class="beans.ChooseForumBean" scope="session" />
<jsp:useBean id="search" class="beans.SearchTopicBean" scope="request" />
...
<head>
	...
	out.println(session.getAttribute("forum"));
	%>-Forum</title>
	<style>
.header_table{
	...
	<tr>
		...
		<td valign="top" width="auto" style="background-color:#EBF0F9;">
			<table align="left">
				<tr>
					<td>
					<form action="SearchTopicAction" method="post" >
						<table width="650px">
				<tr>
								<td width="40px">
	...
	<%
		...
		out.println(session.getAttribute("fullname"));
		 ...
		 %></td>
							...
							<tr>
							<td colspan="3" >
							<div class="forum_labels" >Search Topic
							<input type="text" name="keyword" />
							<input type="submit" value="Search" class="forum_button" />
							<a href="CreateTopic.jsp" class="forum_button" >Create Topic</a>
							</div>
							...
							%>
							<a href="ChooseTopicAction?name=<jsp:getProperty property="topic_name" name="search"/>" >
							<jsp:getProperty property="topic_name" name="search"/>
							</a>
							<%
							}
							else
							...
							<tr>
							<td colspan="3">
							<div class="forum_labels">My Topics:
							</div>
							</td>
							</tr>
							<tr>
							<td colspan="3" >
							<%
							if(request.getAttribute("topiccreated")!=null)
							{
							if(request.getAttribute("topiccreated").equals(true))
							{
							out.println("Your Topic is created");
							%>
							<a href="ChooseTopicAction?name=<jsp:getProperty property="name" name="create"/>" >
							<jsp:getProperty property="name" name="create"/>
							</a>
							<%
							}
							else
							{
							out.println("Your Topic is not created due to some error");
							}
							}
							%>
							<div class="forum_labels" >
							<%
							if(session.getAttribute("mytopic")!=null)
							{
							if(session.getAttribute("mytopic").equals(true))
							{
							Iterator it=chooseforum.getTopicnames().iterator();
							while(it.hasNext())
							{
							beans.MyTopicNames names=(beans.MyTopicNames)it.next();
							%>
							</div>
							</td>
							</tr>
							<tr>
							<td colspan="3" >
							<a href="ChooseTopicAction?name=<%=names.getTopic_name() %>" >
							<%=names.getTopic_name() %>
							</a>
							<%
							}
